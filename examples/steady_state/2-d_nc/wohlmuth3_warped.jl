...
u = range(-1, 1; length=N_elem_i+1)  # uniform in [-1, 1]

# centers of refinement
c1 = 0.25
c2 = 0.75

# cluster strength
ε = 0.15  # amplitude of refinement
w = u .+ ε .* sin.(2π .* u)

dist = collect((w .- minimum(w)) ./ (maximum(w) - minimum(w)))     # warp & map to [0, 1]
